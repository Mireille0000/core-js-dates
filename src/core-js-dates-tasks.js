...
function dateToTimestamp(date) {
  ...
  const milliseconds = currentDate - timeStamp;
  return milliseconds;
}
...
function getTime(date) {
  ...
  return formatedDate.toString().split(' ')[4];
}
...
function getDayName(date) {
  ...
  const getDay = dateArg.getDay();
  return months[getDay];
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const daysStart = new Date(dateStart).getTime() / 1000 / 60 / 60 / 24;
  const daysEnd = new Date(dateEnd).getTime() / 1000 / 60 / 60 / 24;
...
function getCountWeekendsInMonth(month, year) {
  ...
  const daysPerMonth = date.getDate();
  const dArray = JSON.stringify(date).split('-');
  let weekends = 0;

  for (let i = 0; i <= daysPerMonth; i += 1) {
    const day = `${dArray[0]}-${dArray[1]}-${i}`;
    const weekendDay = new Date(day).getDay();
